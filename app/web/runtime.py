...
class WebRuntime:
    ...
    def list_capture_interfaces(self) -> List[Dict[str, Any]]:
        ...
        rows.sort(key=self._preferred_capture_interface_sort_key)
        return rows

    def get_capture_interface_inventory(self) -> Dict[str, Any]:
        rows = self.list_capture_interfaces()
    ...
    def start_passive_capture_scan_job(
        ...
        available_interfaces = {
            ...
            for item in self.list_capture_interfaces()
        }
        ...
        job = self._start_job(
            ...
            lambda job_id: self._run_passive_capture_scan(
                interface_name=resolved_interface,
                duration_minutes=resolved_duration,
                run_actions=bool(run_actions),
                job_id=int(job_id or 0),
    ...
    def _mark_discovered_host_origin(self, host_tokens: List[str], *, source_tool_id: str = ""):
        ...
        for host_token in list(host_tokens or []):
            host = self._resolve_host_by_token(str(host_token or "").strip())
            if host is None:
                continue
            host_id = int(getattr(host, "id", 0) or 0)
            if host_id <= 0:
    ...
    def start_httpx_bootstrap_job(self, targets: List[str]) -> Dict[str, Any]:
        ...
        return self._start_job(
            "httpx-bootstrap",
            lambda job_id: self._run_httpx_bootstrap(normalized_targets, job_id=int(job_id or 0)),
            payload={
    ...
    def _run_httpx_bootstrap(self, targets: List[str], *, job_id: int = 0) -> Dict[str, Any]:
        ...
        for host_token in normalized_targets:
            ...
            command = self._httpx_bootstrap_command(targets_file, output_prefix)
            executed, reason, process_id, metadata = self._run_command_with_tracking(
            ...
            host = self._resolve_host_by_token(str(host_token))
            host_id = int(getattr(host, "id", 0) or 0)
            host_ip = str(getattr(host, "ip", "") or host_token).strip()
            hostname = str(getattr(host, "hostname", "") or host_ip).strip()

            materialized = self._materialize_httpx_urls_as_web_targets(
                host_id=host_id,
                host_ip=host_ip,
                hostname=hostname,
            ...
            if host_id > 0:
                ...
                command_signature = self._command_signature_for_target(command, "tcp")
                if materialized_targets:
        ...
        if materialized_host_ids:
            scheduler_followup = self._run_scheduler_actions_web(
    ...
    def _ingest_discovered_hosts(self, discovered_hosts: List[str], *, source_tool_id: str = "") -> Dict[str, Any]:
        ...
        self._mark_discovered_host_origin(added_hosts, source_tool_id=source_tool_id)
    ...
    def _run_nmap_scan_and_import(
        ...
        try:
            if str(scan_mode or "").strip().lower() == "rfc1918_discovery":
                result = self._run_rfc1918_chunked_scan_and_import(
                    targets=targets,
                    discovery=bool(discovery),
                    run_actions=bool(run_actions),
                    nmap_path=nmap_path,
    ...
    def _run_rfc1918_chunked_scan_and_import(
        ...
        chunk_concurrency = self._normalize_rfc_chunk_concurrency(
            normalized_scan_options.get("chunk_concurrency", 1)
        )
        normalized_scan_options["chunk_concurrency"] = chunk_concurrency

        batches = list(self._iter_rfc1918_scan_batches(targets))
        ...
        def _run_rfc_batch(batch_index: int, batch_targets: List[str]) -> Dict[str, Any]:
            ...
            target_label = self._compact_targets(batch_targets)
        ...
        with ThreadPoolExecutor(max_workers=active_workers, thread_name_prefix="legion-rfc1918") as pool:
            ...
            while pending:
                ...
                self._import_nmap_xml(xml_path, run_actions=False)
        ...
        scheduler_result = self._run_scheduler_actions_web() if run_actions else None
        with self._lock:
            project = self._require_active_project()
            host_count_after = len(project.repositoryContainer.hostRepository.getAllHostObjs())
        imported_hosts = max(0, int(host_count_after) - int(host_count_before))
    ...
    def _analyze_passive_capture(
        ...
        connected_networks = self._connected_ipv4_networks_for_interface(interface_name)
        ...
        for line in str(result.stdout or "").splitlines():
            ...
            for value in (ip_src, ip_dst, arp_src, arp_dst):
                ip_values.extend(self._parse_tshark_field_blob(value))
            ...
            protocols = self._classify_passive_protocols(
                protocol_blob,
                self._parse_tshark_field_blob(udp_src) + self._parse_tshark_field_blob(udp_dst),
                query_name,
            )
    ...
    def _run_passive_capture_scan(
        ...
        capture_filter = self._passive_capture_filter()
        capture_command = self._join_shell_tokens([
            "tshark",
            "-i",
            resolved_interface,
            "-n",
        ...
        analysis = self._analyze_passive_capture(
            interface_name=resolved_interface,
        ...
        self._emit_ui_invalidation("processes", "jobs", "scan_history", "overview")
        return {
            "interface_name": resolved_interface,
            "duration_minutes": resolved_duration,
    ...
    def _build_nmap_scan_plan(
        ...
        if selected_mode == "rfc1918_discovery":
            ...
            default_ports = self.RFC1918_COMPREHENSIVE_TCP_PORTS if rfc_profile == "comprehensive" else self.INTERNAL_QUICK_RECON_TCP_PORTS
            ...
            options["chunk_concurrency"] = self._normalize_rfc_chunk_concurrency(
    ...
    def _record_scan_submission(
        ...
        with self._lock:
            ...
            record = record_scan_submission(project.database, {
                ...
                "status": "submitted",
                "target_summary": str(target_summary or self._compact_targets(normalized_targets)),
                "scope_summary": str(scope_summary or self._summarize_scan_scope(normalized_targets)),
                "targets": normalized_targets,
    ...
    def _count_rfc1918_scan_batches(cls, targets: List[str]) -> int:
        chunk_prefix = max(1, min(int(cls.RFC1918_SWEEP_CHUNK_PREFIX), 32))
        batch_size = max(1, int(cls.RFC1918_SWEEP_BATCH_SIZE))
        chunk_count = 0
    ...
    def _normalize_rfc_chunk_concurrency(cls, raw: Any) -> int:
        try:
            ...
            return 1
        return max(1, min(value, int(cls.RFC1918_SWEEP_MAX_CONCURRENCY)))
    ...
    def _process_progress_adapter_for_command(cls, tool_name: str, command: str) -> str:
        ...
        if cls._is_nuclei_command(tool_name, command):
            return "nuclei"
        if cls._is_tshark_passive_capture_command(tool_name, command):
            return "tshark"
        return ""
    ...
    def _extract_tshark_passive_progress(
        ...
        elapsed_label = cls._format_duration_label(int(bounded_elapsed))
    ...
    def _update_process_progress(
        ...
        if adapter == "nmap":
            ...
            percent, remaining, message = self._extract_tshark_passive_progress(
                command,
                runtime_seconds=runtime_seconds,
            )
        else:
